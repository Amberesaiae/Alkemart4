...
export function MarketAddressFields({
  ...
  return (
    <div className="space-y-3">
      ...
      {locale?.address.help ? (
        <p className="text-sm text-muted-foreground">{locale.address.help}</p>
      ) : null}
      {locale?.phone.hint && !fields.some((f) => f.key === "phone" && f.hint) ? (
        <p className="text-sm text-muted-foreground">{locale.phone.hint}</p>
      ) : null}
      <p className="text-sm text-muted-foreground">
        <strong>{market.currency_code.toUpperCase()}</strong>
        {locale?.payments.preferred?.length ? (
          <>
            {" "}
            · {locale.payments.preferred.join(", ").replace(/_/g, " ")}
          </>
        ) : null}
      </p>
    </div>
