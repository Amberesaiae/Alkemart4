...
const AVAILABLE_ASSETS = new Set<string>([
  "add-cart",
  "badge",
  "cart",
  "cat-all",
...
export function Icon({
  ...
  const hasAsset = preferAsset && AVAILABLE_ASSETS.has(name)
  if (hasAsset) {
    return (
 ...
 */
/** Map pixel sizes → CSS classes (no inline style). */
function iconSlotClass(size: number): string {
  if (size <= 16) return "icon-slot icon-slot-16"
  if (size <= 18) return "icon-slot icon-slot-18"
  if (size <= 20) return "icon-slot icon-slot-20"
  if (size <= 22) return "icon-slot icon-slot-22"
  if (size <= 24) return "icon-slot icon-slot-24"
  if (size <= 28) return "icon-slot icon-slot-28"
  return "icon-slot icon-slot-32"
}
...
export function IconSafe(props: Props) {
  ...
  return (
    <span className={cn(iconSlotClass(size), className)}>
