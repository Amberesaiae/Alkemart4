...
function BrowseRedirect() {
  ...
  const targetSearch: Record<string, unknown> = {}
  if (search.seller) targetSearch.seller = [search.seller]
  if (search.sort) targetSearch.sort = search.sort
  return (
    <Navigate to="/categories/$slug" params={{ slug }} search={targetSearch} replace />
  )
}
