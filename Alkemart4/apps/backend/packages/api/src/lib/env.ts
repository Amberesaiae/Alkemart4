...
export function loadAppEnv(raw: NodeJS.ProcessEnv = process.env): AppEnv {
  const isBuild = process.argv.some((a) => a === "build" || a.endsWith("/medusa") || a.endsWith("\\medusa"))
  const parsed = EnvSchema.safeParse(raw)
  if (!parsed.success) {
    if (isBuild) {
      // During `medusa build`, runtime env vars aren't set. Provide safe defaults
      // so TypeScript compilation and dashboard bundling can proceed.
      const defaults = {
        NODE_ENV: "production" as const,
        DATABASE_URL: "postgresql://build:build@localhost:5432/build",
        REDIS_URL: "redis://localhost:6379",
        JWT_SECRET: "build-placeholder-secret-at-least-32-characters",
        COOKIE_SECRET: "build-placeholder-secret-at-least-32-characters",
        STORE_CORS: "https://build.placeholder",
        ADMIN_CORS: "https://build.placeholder",
        AUTH_CORS: "https://build.placeholder",
        VENDOR_CORS: "https://build.placeholder",
      }
