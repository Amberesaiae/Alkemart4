    ...
    for (const table of TABLE_ORDER) {
      try {
        ...
        for (let i = 0; i < placeholders.length; i += CHUNK) {
          ...
          const chunkVals = values.slice(0, chunk.length * cols.length)
          values.splice(0, chunk.length * cols.length)
          await dst.query(`INSERT INTO "${table}" (${colList}) VALUES ${chunk.join(",")} ON CONFLICT DO NOTHING`, chunkVals)
        }
