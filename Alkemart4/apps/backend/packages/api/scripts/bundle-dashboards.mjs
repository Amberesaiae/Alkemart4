...
const PANELS = [
  ...
  // The vendor panel was replaced with the Ghana vendor app (apps/ghana-vendor).
  // medusa-config.ts vendor-ui appDir resolves via dashboardAppDir('ghana-vendor').
  { name: 'ghana-vendor', basePath: '/seller/' },
]
...
for (const field of ['dependencies', 'devDependencies', 'peerDependencies']) {
  for (const [dep, version] of Object.entries(artifactPkg[field] ?? {})) {
    ...
    // workspace protocol, monorepo "*" links, and private @acme/* packages
    if (
      v.startsWith('workspace:') ||
      dep.startsWith('@acme/') ||
      (v === '*' && dep.startsWith('@acme/'))
    ) {
      delete artifactPkg[field][dep]
    }
  }
}
// Runtime image never needs test tooling
delete artifactPkg.devDependencies
