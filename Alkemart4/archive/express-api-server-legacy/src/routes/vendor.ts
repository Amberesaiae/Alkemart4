...
const UpdateVendorShopBody = z.object({
...
/** Vendor owners can edit public store identity (name, bio). */
router.patch("/vendor/shop", requireAbility("manage", "Vendor"), async (req, res): Promise<void> => {
  const body = UpdateVendorShopBody.safeParse(req.body ?? {});
  if (!body.success) {
  ...
  if (!(await isVendorActive(vendorId))) {
    res.status(403).json({ error: "Vendor account is suspended" });
    return;
  }

  // Enforce ownership: manage Vendor is CASL-scoped, but we still scope the write.
  if (!req.ability.can("manage", "Vendor")) {
