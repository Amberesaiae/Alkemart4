...
import { pesewasToCodeLabel } from "./money";

/**
 * Lightweight in-process domain event dispatcher (Medusa's event-bus pattern,
 * without Redis/queue infra). This is the seam future modules attach subscribers
 * to, without coupling checkout/order logic directly to notification-sending code.
 *
 * When OUTBOX_ENABLED=true, events are also persisted to the transactional outbox
 * for multi-instance durability. The in-process emitter still fires for immediate
 * single-process side effects (notifications, email).
 */
    ...
    try {
      ...
      await sendEmail({
        ...
        html: `
          ...
          <p><strong>Total: ${pesewasToCodeLabel(payload.totalPesewas)}</strong></p>
          <p>${paymentLine}</p>
          <p>Delivery fees and taxes (if any) are included when calculated at checkout.</p>
