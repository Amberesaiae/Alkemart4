...
export type MedusaProduct = {
  ...
  variants?: {
    ...
    /** Mercur: cheapest offer id when fields include variants.offer_id / calculated_price */
...
function medusaToAlkemartProduct(p: MedusaProduct): AlkemartProduct {
  ...
  const offerId =
    typeof variant?.offer_id === "string" && variant.offer_id.trim()
      ? variant.offer_id.trim()
      : null
...
function medusaToAlkemartListItem(p: MedusaProduct): AlkemartProductListItem {
  ...
  // Keep stock for PLP honesty; drop only heavy detail fields.
  const { description: _d, sku: _sku, ...rest } = full
  return rest
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const query: Record<string, string | number | string[]> = {
        limit,
        offset,
        // Mercur hydrates variants.offer_id with calculated_price.
        // Do NOT request bare `variants.offer_id` — it breaks store list (empty/null titles).
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const query: Record<string, string> = {
        fields:
          "*variants.calculated_price,+variants.inventory_quantity,*images,*categories,*collection,*tags",
      }
      const regionId = commerceContext.tryRegionId()
      if (regionId) query.region_id = regionId

      // Prefer retrieve by id or handle.
      try {
        const { product } = await sdk.store.product.retrieve(idOrHandle, query as any)
        if (product) return medusaToAlkemartProduct(product as MedusaProduct)
      } catch {
        // fall through — try list filters
      }

      // Fallbacks: handle match, then id match (Mercur/store quirks).
      const listBase: Record<string, string | number> = {
        limit: 1,
        fields: query.fields,
      }
      if (regionId) listBase.region_id = regionId

      for (const filter of [{ handle: idOrHandle }, { id: idOrHandle }] as const) {
        try {
          const response = await sdk.store.product.list({ ...listBase, ...filter } as any)
          const hit = (response.products ?? [])[0] as MedusaProduct | undefined
          if (hit) return medusaToAlkemartProduct(hit)
        } catch {
          // try next filter
        }
      }
