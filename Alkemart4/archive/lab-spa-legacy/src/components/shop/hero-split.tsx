...
export function HeroSplit({
  ...
  return (
    <div
      ...
      style={{
        minHeight,
        ...(hasThemeOverride ? { backgroundColor: themeBg, color: themeFg } : undefined),
      }}
    >
      {imagePosition === "left" && (
      ...
      <div className="flex flex-col justify-center gap-6 p-10">
        {eyebrow && (
          <div className="text-xs font-bold uppercase tracking-widest opacity-80">{eyebrow}</div>
        )}
        <h2 className="text-4xl font-bold leading-[1.05] tracking-tight md:text-5xl">{title}</h2>
        <div>
          <Button size="lg" variant={tone === "primary" ? "accent" : "default"} asChild>
