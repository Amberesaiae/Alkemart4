...
export async function paystackRequest<T = Record<string, unknown>>(
  ...
  const json = (await res.json().catch(() => null)) as {
    status?: boolean
    message?: string
    data?: T & { message?: string; status?: string }
  } | null

  if (!res.ok || !json?.status) {
    // Prefer nested gateway/decline detail over generic "Charge attempted"
    const nested =
      json?.data &&
      ...
      typeof (json.data as { message?: unknown }).message === "string"
        ...
        : null
    const message =
      nested || json?.message || `Paystack API error (${res.status})`
