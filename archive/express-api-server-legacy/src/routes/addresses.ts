...
import { DEFAULT_COUNTRY_CODE } from "../lib/platform-config";
...
router.post("/addresses", requireAuth, async (req, res): Promise<void> => {
  ...
  const created = await db.transaction(async (tx) => {
    ...
    const [row] = await tx
      ...
      .values({
        ...body.data,
        countryCode: body.data.countryCode?.toUpperCase() ?? DEFAULT_COUNTRY_CODE,
...
router.patch("/addresses/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const updated = await db.transaction(async (tx) => {
    ...
    const patch = { ...body.data } as typeof body.data & { countryCode?: string };
