...
const MODES: { id: Mode; label: string; hint: string }[] = [
  { id: "delivery", label: "Delivery", hint: "To your address" },
  { id: "pickup", label: "Pickup", hint: "At a store" },
  { id: "shipping", label: "Shipping", hint: "Standard ship" },
];
...
export function LocationChip({ className }: LocationChipProps) {
  ...
  if (isPickup) {
    ...
    // Short on purpose — fits the chip without "Sign in for deliv…"
    destination = "Sign in";
  }
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="start" sideOffset={10} className="w-[22rem] p-4 shadow-lg">
        <div className="mb-3">
          <p className="text-sm font-semibold">How do you want your order?</p>
          <p className="text-xs text-muted-foreground">Choose delivery, pickup, or shipping.</p>
        </div>
          ...
          <div className="space-y-2">
            ...
            {PICKUP_STORES.length === 0 ? (
              <div className="rounded-lg border border-dashed border-border bg-muted/40 px-3 py-4 text-sm text-muted-foreground">
                Pickup stores are not listed yet. Use delivery or shipping with a saved address for now.
              </div>
            ) : (
