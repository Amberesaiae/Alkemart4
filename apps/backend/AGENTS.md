...
- Backend tests: Jest (`bun run test:unit`, `bun run test:integration:modules`) from `packages/api`
- Storefront tests: Vitest (`bun run test`) from `apps/storefront`
- CRITICAL paths must include tests: auth, checkout, payment webhooks, env validation

## Security

- Never commit `.env` files to git (they are gitignored)
- All secrets via environment variables, validated on boot via Zod schema
