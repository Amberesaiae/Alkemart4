/**
 * Same language set as Mercur Seller Hub AuthLanguageSelect.
 * Codes must match i18n resource keys in @mercurjs/admin.
 */
...
/** Medusa/Mercur i18n detector keys */
export const LNG_STORAGE_KEY = "lng"
export const LNG_COOKIE = "lng"

export function readStoredLanguage(): string {
  try {
    const fromLs = localStorage.getItem(LNG_STORAGE_KEY)
...
export function persistLanguage(code: string) {
  ...
  document.cookie = `${LNG_COOKIE}=${encodeURIComponent(code)}; path=/; max-age=31536000; SameSite=Lax`
